...
import com.atminzhi.mongotest.entity.ChinaDate;
...
public class ChinaDateCame {
    ...
    public String getVocationName(DomNodeList<HtmlElement> htmlElements, String date) throws ParseException {
        System.out.println("传入的时间为："+date); //传入的时间为：2021/2/11-17 春节
        String rst = "";
...
//from the latest Vocation name
            rst = ChinaDateCame.latestVocationName;
        }else if(pastTimeFlag == true){
//更新《当前时间，且最近一次的可见的假期名
            ChinaDateCame.latestVocationName = rst;
        }
        return rst;
    }
...
//                    Date parse = dateFormat1.parse("2021-02-27 12:23:25");
//                    String format = dateFormat1.format(parse);
                    chinaDate.setVacationName(this.getVocationName(htmlElements, element.getAttribute("date")));//element.getAttribute("format")
                }
    ...
    public static void main(String[] args) throws FailingHttpStatusCodeException, MalformedURLException, IOException, InterruptedException {
        //得到当前日期信息
        List<ChinaDate> dateList = new ChinaDateCame().getCurrentDateInfo();
        //得到今天日期信息
        ChinaDate today = new ChinaDateCame().getTodayInfo();
        DateFormat dateFormat = new SimpleDateFormat("yyyy/MM/dd");
        ...
        System.out.println("已发生的最近一次假期:" + ChinaDateCame.latestVocationName);
    }
}
